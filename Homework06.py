...
class Publisher(Base):
    ...
    name = sq.Column(sq.String(length=28), unique=True)
...
class Shop(Base):
    ...
    id = sq.Column(sq.Integer, primary_key=True)
    name = sq.Column(sq.String(length=28), nullable=False)
...
class Stock(Base):
    ...
    shop = relationship(Shop, backref="stockus")
...
class Sale(Base):
    ...
    def __str__(self):
        return f'Sale{self.id}: ({self.price}, {self.date_sale}, {self.stock_id}, {self.sale_count})'
